package gmedia.net.id.coremodul;

/**
 * Created by Shin on 08/09/2017.
 */

public class FormatItem {

    public static final String formatDate = "yyyy-MM-dd";
    public static final String formatTimestamp = "yyyy-MM-dd HH:mm:ss";
    public static final String formatTimestamp2 = "yyyyMMddHHmmss";
    public static final String formatDateTime = "yyyy-mm-dd HH:mm:ss";
    public static final String formatDateTime2 = "yyyy-mm-dd HH:mm";
    public static final String formatTime = "HH:mm:ss";
    public static final String formatTime2 = "HH:mm";
    public static final String formatDateDisplay = "dd/MM/yyyy";
    public static final String formatDateTimeDisplay = "dd/MM/yyyy HH:mm";
    public static final String formatDateTimeDisplay1 = "dd/MM/yyyy HH:mm:ss";
    public static final String formatDateTimeDisplay2 = "dd MMM yyyy HH:mm:ss";
    public static final String formatDateTimeDisplay3 = "dd MMM yyyy HH:mm";
    public static final String formatDate1 = "dd MMM yyyy";
    public static final String formatTimeCalender ="EEE MMM dd HH:mm:ss zzzz yyyy";

}
